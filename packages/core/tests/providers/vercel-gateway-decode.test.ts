...
describe('vercel-gateway observations', () => {
  ...
  it('the hostile envelope serializes with none of the planted secrets outside the dedup key', () => {
    ...
    // neither may survive the observation boundary. The dedup key
    // (`vercel-gateway:<day>:<model>`) still carries the report's RAW model:
    // normalization is boundary-only by design, and the key's free-text surface
    // is closed schema-wide by hashing dedup keys, not by cleaning one
    // component of one provider's key.
    const serialized = JSON.stringify(buildEnvelope(), (k, v) => (k === 'dedupKey' ? '<dedupKey>' : v))
