...
describe('antigravity rich decode (moved to @codeburn/core)', () => {
  ...
  it('acceptance: a display-name model read from payload.model.display_name is normalized at the observation boundary', () => {
    ...
    // Boundary only: the observation's model field is clean. The decoder's
    // usageSignature (and therefore the dedup key) is deliberately built from
    // the RAW display name — normalizing a key component would merge distinct
    // calls and churn every cached key; the key's free-text surface is closed
    // schema-wide by hashing dedup keys, not here.
    expect(JSON.stringify(envelope, (k, v) => (k === 'dedupKey' ? '<dedupKey>' : v)))
      .not.toContain('Gemini 3.5 Flash')
  })
