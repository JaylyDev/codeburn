 ...
 * import time at any exports subpath, nor when ONE REPRESENTATIVE parser,
 * decoder, and detector body is actually called (claude parseJsonlLine, codex
 * decodeCodex, junkReadsDetector). Import purity is proven for every subpath;
 * call-time purity is proven for those three only — it is a spot check, not a
 * per-provider sweep. We run
