// Minimizing transform: rich Gemini decode -> the strict observation envelope.
...
import type { GeminiDecodedCall } from './types.js'

/** One Gemini session's rich decode, as the host holds it before minimization. */
export interface RichGeminiSessionDecode {
  sessionId: string
  /** Absolute project path (the session cwd); fingerprinted, never emitted raw. */
  projectPath: string
  /** Rich, cost-free calls in decode order (as decodeGemini emits them). */
  calls: GeminiDecodedCall[]
}

export interface GeminiToObservationsContext {
...
function toCallObservation(call: GeminiDecodedCall, turnIndex: number, privacyKey: string): CallObservation {
  return {
...
function toSessionObservation(decode: RichGeminiSessionDecode, ctx: GeminiToObservationsContext): SessionObservation {
...
/**
 * Map a rich Gemini decode (one or many sessions) into the minimized observation
...
export function toObservations(
  decode: RichGeminiSessionDecode | RichGeminiSessionDecode[],
  ctx: GeminiToObservationsContext,
