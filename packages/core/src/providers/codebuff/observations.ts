// Minimizing transform: rich Codebuff decode -> the strict observation envelope.
...
import type { CodebuffDecodedCall } from './types.js'

/** One Codebuff session's rich decode, as the host holds it before minimization. */
export interface RichCodebuffSessionDecode {
  sessionId: string
  /** Absolute project path; fingerprinted, never emitted raw. */
  projectPath: string
  /** Rich, cost-free calls in decode order (as decodeCodebuff emits them). */
  calls: CodebuffDecodedCall[]
}

export interface CodebuffToObservationsContext {
...
function toCallObservation(call: CodebuffDecodedCall, turnIndex: number, privacyKey: string): CallObservation {
  return {
...
function toSessionObservation(decode: RichCodebuffSessionDecode, ctx: CodebuffToObservationsContext): SessionObservation {
  const provider = ctx.provider ?? 'codebuff'
...
/**
 * Map a rich Codebuff decode (one or many sessions) into the minimized observation
...
export function toObservations(
  decode: RichCodebuffSessionDecode | RichCodebuffSessionDecode[],
  ctx: CodebuffToObservationsContext,
