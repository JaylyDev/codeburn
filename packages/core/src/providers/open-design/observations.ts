...
import type { OpenDesignDecodedCall } from './types.js'

export interface RichOpenDesignSessionDecode {
  sessionId: string
  projectPath: string
  calls: OpenDesignDecodedCall[]
}

export interface OpenDesignToObservationsContext {
  privacyKey: string
  provider?: string
}
...
function toCallObservation(call: OpenDesignDecodedCall, turnIndex: number, privacyKey: string): CallObservation {
  return {
...
function toSessionObservation(decode: RichOpenDesignSessionDecode, ctx: OpenDesignToObservationsContext): SessionObservation {
  const provider = ctx.provider ?? 'open-design'
...
export function toObservations(
  decode: RichOpenDesignSessionDecode | RichOpenDesignSessionDecode[],
  ctx: OpenDesignToObservationsContext,
