// Minimizing transform: rich Kimicode decode -> the strict observation envelope.
...
/** One Kimicode session's rich decode, as the host holds it before minimization. */
export interface RichKimicodeSessionDecode {
  sessionId: string
  /** Absolute project path (the session cwd); fingerprinted, never emitted raw. */
  projectPath: string
  /** Rich, cost-free calls in decode order (as decodeKimicode emits them). */
  calls: KimicodeDecodedCall[]
}

export interface KimicodeToObservationsContext {
...
function toSessionObservation(decode: RichKimicodeSessionDecode, ctx: KimicodeToObservationsContext): SessionObservation {
  const provider = ctx.provider ?? 'kimicode'
...
/**
 * Map a rich Kimicode decode (one or many sessions) into the minimized observation
...
export function toObservations(
  decode: RichKimicodeSessionDecode | RichKimicodeSessionDecode[],
  ctx: KimicodeToObservationsContext,
