// Minimizing transform: rich Droid decode -> the strict observation envelope.
...
function toSessionObservation(decode: RichDroidSessionDecode, ctx: DroidToObservationsContext): SessionObservation {
  const provider = ctx.provider ?? 'droid'
...
export function toObservations(
  decode: RichDroidSessionDecode | RichDroidSessionDecode[],
  ctx: DroidToObservationsContext,
