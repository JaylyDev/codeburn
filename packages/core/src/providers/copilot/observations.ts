// Minimizing transform: rich Copilot decode -> the strict observation envelope.
...
/** One Copilot session's rich decode, as the host holds it before minimization. */
...
export interface CopilotToObservationsContext {
...
/**
 * Map a rich Copilot decode (one or many sessions) into the minimized observation
 * layer. Returns the `sessions` array plus any per-record `diagnostics`.
 */
export function toObservations(
  decode: RichCopilotSessionDecode | RichCopilotSessionDecode[],
  ctx: CopilotToObservationsContext,
