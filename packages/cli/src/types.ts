...
export type ParsedApiCall = {
  ...
  toolErrors?: number
  /// Tool-excluded active throughput (Codex only): the task duration minus its
  /// recorded tool-wait intervals, the task's generated tokens, and the excluded
  /// wait share, attributed to this call proportionally. Omitted when the task
  /// recorded no timing.
