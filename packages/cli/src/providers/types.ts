...
export type ParsedProviderCall = {
  ...
  // Tool-excluded active throughput: `activeDurationMs` is the enclosing task's
  // duration minus its recorded tool-wait intervals, `activeGeneratedTokens` the
  // task's generated tokens, both attributed to this call proportionally (Codex
  // only). `toolWaitMs` is the excluded wait share. Present only when the task
  // recorded both timing and generated tokens.
