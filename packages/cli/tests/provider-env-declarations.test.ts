...
// from the real code on the extraction branch (packages/cli); registry names
// come from src/providers/index.ts. Do NOT infer this from the filename at
// runtime — the two diverge (e.g. the shared sqlite-session-parser.ts serves
// two providers). A file that contains env reads and is missing here fails
...
const FILE_PROVIDERS: Record<string, string[]> = {
  ...
  // Its only literal read is CODEBURN_CURSOR_MAX_BUBBLES (cursor.ts:530).
  'cursor.ts': ['cursor'],
  // The ENV_DIR const (open-design.ts:11) resolves to CODEBURN_OPEN_DESIGN_DIR.
  ...
  // the cached report (parser.ts:1442/1455) cannot keep serving the previous
