// #1083, cost site 2 of 2. Codex is NOT on parser.ts's reported-cost
...
// buckets in cachedCallToApiCall. That line and the one in src/pricing-pass.ts
...
const testRoot = vi.hoisted(() => {
  const root = `${process.env['TMPDIR'] || '/tmp'}/codex-1083-rehydrate-${process.pid}-${Date.now()}`
...
it('prices a codex call the same on a cold parse and a cache-rehydrated read', async () => {
  ...
  await writeFile(join(sessionDir, 'rollout-1083.jsonl'), [
    JSON.stringify({ type: 'session_meta', timestamp: '2026-08-16T10:00:00Z', payload: { session_id: 's1083', model: 'gpt-5.5', cwd: '/Users/test/proj', originator: 'codex_cli_rs' } }),
  ...
  // Drop the in-memory cache only: the on-disk session cache now serves the
  ...
  // Revert only src/pricing-pass.ts and the cold leg breaks; revert only
