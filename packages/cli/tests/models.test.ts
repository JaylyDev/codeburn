...
describe('pricing cache schema version (#1078 follow-up, ported in #1083)', () => {
  it('discards a cache written by a pre-fix binary instead of reading its missing cacheWriteCostIsExplicit as false', async () => {
    const cacheRoot = await mkdtemp(join(tmpdir(), 'codeburn-pricing-cache-'))
    try {
      process.env['CODEBURN_CACHE_DIR'] = cacheRoot
      await mkdir(cacheRoot, { recursive: true })
      // Shape of a cache file written before the fix added `version` and
      ...
      // (falsy) here instead of the true its snapshot entry actually carries.
