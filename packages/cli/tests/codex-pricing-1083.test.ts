// Regression suite for #1083 — the branch port of #1075/#1078.
//
...
//      On this branch the fresh-parse price lives in src/pricing-pass.ts and
//      the cache-rehydration re-price in src/parser.ts; both now go through
//      billableOutputTokens(). The rehydration half lives in
//      codex-pricing-1083-rehydrate.test.ts, which needs CODEX_HOME set before
//      the provider module is evaluated.
//
//   B. cache_write_input_tokens was never read. The core decoder now reads and
//      clamps it, and the host adapter carves it out of the uncached-input
//      bucket — but ONLY on models whose pricing source carries an explicit
//      cache-write rate: buildCosts() fabricates 1.25x input when the source
//      omits one, which is right for Anthropic but would invent a surcharge
//      OpenAI never charged on every pre-5.6 model.
...
//   gpt-5.6-terra  input 2.5e-6  output 15e-6  cacheWrite 3.125e-6 (EXPLICIT)  cacheRead 2.5e-7
//   gpt-5.5        input 5e-6    output 30e-6  cacheWrite null (fabricated)    cacheRead 5e-7
const TERRA = { input: 2.5e-6, output: 15e-6, cacheWrite: 3.125e-6, cacheRead: 2.5e-7 }
const GPT55 = { input: 5e-6, output: 30e-6, cacheRead: 5e-7 }

let tmpDir: string
beforeEach(async () => { tmpDir = await mkdtemp(join(tmpdir(), 'codex-1083-')) })
...
async function parseOneEvent(model: string, usage: Usage): Promise<ParsedProviderCall> {
  ...
  await writeFile(filePath, [
    JSON.stringify({
      ...
      payload: { cwd: '/Users/t/p', originator: 'codex-cli', session_id: 's1083', model },
    }),
    JSON.stringify({
      type: 'event_msg',
...
describe('#1083 A - reasoning is not billed on top of output', () => {
  ...
  it('does not double-count reasoning in the displayed output tokens', async () => {
    ...
    // separate from output, unlike codex/claude.
...
describe('#1083 B - cache_write_input_tokens', () => {
  ...
  it('THE GUARD: leaves cache writes in the input bucket when the model has no explicit rate', async () => {
    // gpt-5.5 carries `null` for its cache-write slot, so buildCosts fabricates
    // 1.25x input for it. OpenAI charges nothing extra to write cache before
    // gpt-5.6, so routing these tokens through that fabricated rate would
    // invent a surcharge. Cost must be byte-identical to the pre-fix number.
    // Delete the guard and this test fails.
...
describe('#1083 cache invalidation', () => {
  it('discards a v12 codex results cache (it stores costUSD verbatim)', async () => {
    ...
    const stale: ParsedProviderCall = {
      ...
      reasoningTokens: 400,
      webSearchRequests: 0,
      costUSD: 0.0465, // the pre-fix, reasoning-double-counted number
      tools: [],
      bashCommands: [],
      timestamp: '2026-08-16T10:01:00Z',
      speed: 'standard',
    ...
    await writeFile(join(cacheDir, 'codex-results.json'), JSON.stringify({
      ...
      files: {
        [sessionFile]: {
          mtimeMs: s.mtimeMs, sizeBytes: s.size, project: 'p',
          byteOffset: 0, state: {}, callCount: 1, calls: [stale],
        },
      },
    }))

    process.env['CODEBURN_CACHE_DIR'] = cacheDir
    // Fresh module instance so the module-level mem cache re-reads from disk.
    vi.resetModules()
    const { readCodexCacheEntry } = await import('../src/codex-cache.js')
    // Revert CODEX_CACHE_VERSION to 12 and this returns the stale $0.0465 call.
    expect(await readCodexCacheEntry(sessionFile)).toBeNull()
  })

  it('re-derives days finalized at daily-cache v27', async () => {
    ...
    const oldPath = join(cacheRoot, 'daily-cache.v27.json')
    const oldCache = {
      version: 27,
    ...
    // Drop MIN_SUPPORTED_VERSION back to 27 and the v27 day is trusted as-is,
