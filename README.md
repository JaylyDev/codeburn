...
Only **running** distros are scanned by default: reaching into `\\wsl$\<distro>` boots a stopped distro, which CodeBurn will not do behind your back. Start the distro (or set `CODEBURN_WSL=all`) if you want the others included. `codeburn doctor` lists every root it probed, WSL ones included, so an empty result says which path was missing rather than just zero.
...
| `CODEBURN_WSL=off` | Skips WSL entirely; `wsl.exe` is never run. |
