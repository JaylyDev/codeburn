...
A: On purpose. Copilot input/cache used to sync as one `session.shutdown` rollup span per (session, model); it now syncs as one span per API request. Locally that is a replacement, but a receiver cannot be told to drop a span it already has — the ledger is append-once and usage spans have no retraction — so re-sending the same tokens in their new shape would double them there forever. Any Copilot session whose rollup is already in the ledger is therefore frozen at what was sent: its per-request rows and residuals never go out, and the receiver keeps the older, lossier number. Sessions first seen after the upgrade sync the full breakdown. Per-turn output spans are unaffected either way. `--dry-run` reports the frozen count. `codeburn sync reset --confirm` clears the local ledger and re-pushes everything under the new breakdown — only do that if the receiver's copy is cleared too, or you get exactly the doubling this avoids.
